...
if [ "${RELEASE_CHECK_ORIGIN:-yes}" = "yes" ]; then
  ...
  if [ "${git_origin_url}" != "git@github.com:enactic/dora-openarm-dummy-classifier.git" ]; then
    echo "This script must be ran with working copy of enactic/dora-openarm-dummy-classifier."
...
if [ "${RELEASE_TAG:-yes}" = "yes" ]; then
  echo "Tag"
  git tag -a -m "dora-openarm-dummy-classifier ${version}" "${version}"
